"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var TimeslotsScheduler_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TimeslotsScheduler = void 0;
const common_1 = require("@nestjs/common");
const schedule_1 = require("@nestjs/schedule");
const timeslots_service_1 = require("./timeslots.service");
let TimeslotsScheduler = TimeslotsScheduler_1 = class TimeslotsScheduler {
    constructor(timeslotsService) {
        this.timeslotsService = timeslotsService;
        this.logger = new common_1.Logger(TimeslotsScheduler_1.name);
    }
    async handleDailyTimeslotMaintenance() {
        this.logger.log('Starting daily timeslot maintenance...');
        try {
            const result = await this.timeslotsService.dailyTimeslotMaintenance();
            this.logger.log(`Daily maintenance completed: Deleted ${result.deletedCount} expired timeslots, Added ${result.addedCount} new timeslots`);
        }
        catch (error) {
            this.logger.error('Daily timeslot maintenance failed:', error);
        }
    }
};
exports.TimeslotsScheduler = TimeslotsScheduler;
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_DAY_AT_MIDNIGHT),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], TimeslotsScheduler.prototype, "handleDailyTimeslotMaintenance", null);
exports.TimeslotsScheduler = TimeslotsScheduler = TimeslotsScheduler_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [timeslots_service_1.TimeslotsService])
], TimeslotsScheduler);
//# sourceMappingURL=timeslots.scheduler.js.map